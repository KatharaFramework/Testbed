...
def create_client():
    ...
    with open("client.startup", "w") as startup_file:
        startup_file.write("ifconfig eth0 %s up\n" % client_ip)
        startup_file.write("route add default gw %s dev eth0\n\n" % router_ip.split('/')[0])
...
def create_router(num, assigned_ip=None, collision_domain=None):
    ...
    with open("%s.startup" % router_name, "w") as startup_file:
        startup_file.write("ifconfig eth0 %s up\n" % first_ip)
        startup_file.write("ifconfig eth1 %s up\n\n" % second_ip)

        ip_1 = ipaddress.ip_address(unicode(first_ip.split("/")[0])) - 1
        ip_2 = ipaddress.ip_address(unicode(second_ip.split("/")[0])) + 1
        startup_file.write("route add default gw %s dev eth1\n" % str(ip_2))
        startup_file.write("route add -net 10.0.0.0/24 gw %s dev eth0" % str(ip_1))
...
def create_server(ip, collision_domain):
    ...
    with open("server.startup", "w") as startup_file:
        startup_file.write("ifconfig eth0 %s up\n" % server_ip)
        startup_file.write("route add -net 10.0.0.0/24 gw %s dev eth0\n\n" % ip.split('/')[0])
